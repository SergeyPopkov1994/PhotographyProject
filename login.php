  ...
  if(!isset($_COOKIE['email'])) {
    if(isset($_POST['submit'])) {
      ...
      if(!empty($email) && !empty($password)) {
        $query = "SELECT `user_id`, `email`, `first_name`, `second_name`, `role` FROM `users` WHERE email = '$email' and password = SHA('$password')";
        $data = mysqli_query($dbc, $query);
        if(mysqli_num_rows($data) == 1) {
          $row = mysqli_fetch_assoc($data);
          setcookie('user_id', $row['user_id'], time() + 60 * 60);
          setcookie('email', $row['email'], time() + 60 * 60);
          setcookie('first_name', $row['first_name'], time() + 60 * 60);
          setcookie('second_name', $row['second_name'], time() + 60 * 60);
...
<body style="background: url(../static/img/photo4.jpg) no-repeat; -moz-background-size: 100%;
...
<div class="container">
    <div class="row  justify-content-center" style="margin-top: 100px;">
        <div class="card" style="background: radial-gradient(40% 50%, #FAECD5, #CAE4D8);">
            <h5 class="card-header info-color white-text text-center py-4">
                <strong>Авторизация</strong>
            </h5>
            <div class="card-body px-lg-5 pt-0">
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="text-center" style="color: #757575;">
                    <div class="md-form">
                        <br>
